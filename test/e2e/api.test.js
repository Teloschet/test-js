  ...
  describe('/login', () => {
    ...
    it('should login successfully given user and password', async () => {
      ...
      assert.strictEqual(
        result.status, 
        expected, 
        `status code should be 200, actul: ${result.status}`
      )

      const response = await result.json()
      assert.ok(response.token.length > 20, `response.token should be a valid jwt token. actual: ${JSON.stringify(response)}`)
  ...
  describe('/', () => {
    it('should not be allowed to access private data without a token', async () => {
      ...
      assert.strictEqual(
        result.status, 
        expected, 
        `status code should be 400, actul: ${result.status}`
      )
